...
public class BankCardScannerActivity extends BaseActivity {
...
//        scannerView.setEnableLicensePlate(true);//车牌识别
        scannerView.setEnableBankCard(true);//银行卡识别
        scannerView.setRotateDegree90Recognition(true);
        scannerView.setCallback(new Callback() {
            @Override
            public void result(Result result) {
//                tvResult.setText("识别结果：\n" + result.toString());
                MyLogger.i(">>>>" + result.data);

                Intent resultIntent = new Intent();
                Bundle bundle = new Bundle();
                bundle.putString("bankCard", result.data);
                resultIntent.putExtras(bundle);
                BankCardScannerActivity.this.setResult(RESULT_OK, resultIntent);
                finish();
                /*startVibrator();
