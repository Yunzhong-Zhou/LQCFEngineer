...
public class JinXingZhongActivity extends BaseActivity {
    ...
    TextView tv_carnum, tv_name, tv_time, tv_carname, tv_carpailiang, tv_carnianfen, tv_carlicheng,
            tv_money, tv_beizhu, tv_jiechename, tv_yujitime,tv_id;
    ...
    List<String> list_img_temp = new ArrayList<>();
    //其他商品
    ...
    protected void initView() {
        ...
        tv_yujitime = findViewByID_My(R.id.tv_yujitime);
...
//                showEmptyPage();
            ...
            public void onResponse(OrderDetailModel response) {
                ...
                tv_yujitime.setText(model.getTechn_sedan_info().getEstimateTime().trim());//预计完成时间

                /**
                 * 服务列表
                 */
                mAdapter_service = new CommonAdapter<OrderDetailModel.OrderInfoBean.OrderServiceListBean>(
                        JinXingZhongActivity.this, R.layout.item_service_jinxing, model.getOrder_info().getOrder_service_list()) {
                    @Override
                    protected void convert(ViewHolder holder, OrderDetailModel.OrderInfoBean.OrderServiceListBean bean, int position) {
                        ...
                        holder.setText(R.id.tv_kaishitime, "开始时间：" + bean.getStartTime());//检查时间
                        if (bean.getStartTime() != null && !bean.getStartTime().trim().equals("")) {
                            holder.setText(R.id.tv_kaishitype, "已开始");//检查状态
                        } else {
                            holder.setText(R.id.tv_kaishitype, "未开始");//检查状态
                        }
                        holder.setText(R.id.tv_wangongtime, "完工时间：" + bean.getOkTime());//完工时间
                        if (bean.getOkTime() != null && !bean.getOkTime().trim().equals("")) {
                            holder.setText(R.id.tv_wangongtype, "已完工");//完工状态
                        } else {
                            holder.setText(R.id.tv_wangongtype, "未完工");//完工状态
                        }

//                holder.setText(R.id.serviceallmoney, "¥" + bean.get);
                        //商品列表
                        RecyclerView rv_s = holder.getView(R.id.rv_s);
                        LinearLayoutManager llm1 = new LinearLayoutManager(JinXingZhongActivity.this);
                        ...
                        CommonAdapter<OrderDetailModel.OrderInfoBean.OrderServiceListBean.OrderGoodsListBeanX> ca_s =
                                new CommonAdapter<OrderDetailModel.OrderInfoBean.OrderServiceListBean.OrderGoodsListBeanX>
                                        (JinXingZhongActivity.this, R.layout.item_orderdetail_goods, bean.getOrder_goods_list()) {
                                    @Override
                                    protected void convert(ViewHolder holder, OrderDetailModel.OrderInfoBean.OrderServiceListBean.OrderGoodsListBeanX model1, int p) {
                                        ...
                                        Glide.with(JinXingZhongActivity.this).load(URLs.IMGHOST + model1.getGoods_info().getGImg())
                                                .centerCrop()
...
//                                        String[] strArr = model1.getGoods_info().getImgStr().split("\\|\\|");
                                        ...
                                        CommonAdapter<String> ca = new CommonAdapter<String>
                                                (JinXingZhongActivity.this, R.layout.item_img_80_80, list_img) {
                                            @Override
                                            protected void convert(ViewHolder holder, String model, int position) {
                                                ImageView iv = holder.getView(R.id.iv);
                                                Glide.with(JinXingZhongActivity.this).load(model)
                                                        .centerCrop()
//                            .apply(RequestOptions.bitmapTransform(new RoundedCorners(10)))
                                                        .placeholder(R.mipmap.loading)//加载站位图
                                                        .error(R.mipmap.zanwutupian)//加载失败
                                        ...
                                        ca.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
                                            ...
                                            public void onItemClick(View view, RecyclerView.ViewHolder viewHolder, int i) {
                                                PhotoShowDialog photoShowDialog = new PhotoShowDialog(JinXingZhongActivity.this, list_img, i);
...
//                                        holder.getView(R.id.delete_goods).setVisibility(View.GONE);
                        ...
                        switch (bean.getGState()) {
                            case 1://施工中
                                addgoods.setText("施工完成");
                                addgoods.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View v) {
                                        showToast("确认施工完成了吗？", "确认", "取消", new View.OnClickListener() {
                                            @Override
                                            public void onClick(View v) {
                                                dialog.dismiss();
                                                Map<String, String> params = new HashMap<>();
                                ...
                                break;
                            case 2://已完成
                                addgoods.setText("已完成");
                               /* addgoods.setText("复检完成");
                                addgoods.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View v) {
                                        showToast("确认复检完成了吗？", "确认", "取消", new View.OnClickListener() {
                                            @Override
                                            public void onClick(View v) {
                                                dialog.dismiss();
                                                Map<String, String> params = new HashMap<>();
                                    ...
                                    public void onClick(View v) {
                                        showToast("确认开始施工吗？", "确认", "取消", new View.OnClickListener() {
                                            ...
                                            public void onClick(View v) {
                                                ...
                                                params.put("u_token", localUserInfo.getToken());
                                                params.put("y_order_service_id", bean.getYOrderServiceId());
                                                params.put("g_state", "1");//1为进行施工中 2为已完成  3已检查
                                                RequestChangeServiceType(params);
                                            }
                                        }, new View.OnClickListener() {
                                            @Override
...
//                        holder.getView(R.id.delete_service).setVisibility(View.GONE);
                            ...
                            case 2:
                                ...
                                addgoods.setOnClickListener(new View.OnClickListener() {
                                    ...
                                    public void onClick(View v) {
                                        ...
                                        CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, SelectGoodsActivity.class, bundle2, false);
                                    }
                                });
                                break;
                            case 3:
                        ...
                        //添加商品
                        holder.getView(R.id.addgoods1).setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                Bundle bundle2 = new Bundle();
                                bundle2.putString("y_store_id", model.getOrder_info().getYStoreId());
...
//                        list_img.clear();
                        List<String> list_img = new ArrayList<>();
                        if (bean.getImgstrArr() != null) {
                            for (String s : bean.getImgstrArr()) {
                                if (!s.equals("")) {
//                                    list_img.add(URLs.IMGHOST + s);
                                    list_img.add(s);
                                }
                        ...
                        LinearLayoutManager llm_img = new LinearLayoutManager(JinXingZhongActivity.this);
                        llm_img.setOrientation(LinearLayoutManager.HORIZONTAL);// 设置 recyclerview 布局方式为横向布局
                        rv_img.setLayoutManager(llm_img);
                        CommonAdapter<String> ca_img = new CommonAdapter<String>
                                (JinXingZhongActivity.this, R.layout.item_img_80_80_detele, list_img) {
                            @Override
                            protected void convert(ViewHolder holder, String model, int position) {
                                ImageView iv = holder.getView(R.id.iv);
                                Glide.with(JinXingZhongActivity.this).load(model)
...
//                            .apply(RequestOptions.bitmapTransform(new RoundedCorners(10)))
                        ...
                        ca_img.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
                            @Override
                            public void onItemClick(View view, RecyclerView.ViewHolder viewHolder, int i) {
                                PhotoShowDialog photoShowDialog = new PhotoShowDialog(JinXingZhongActivity.this, list_img, i);
                                photoShowDialog.show();
                        ...
                        rv_img.setAdapter(ca_img);
                        //添加施工图片
                        holder.getView(R.id.addimg).setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                if (list_img.size() < 6) {

                                    list_img_temp = bean.getImgstrArr();
                                    y_order_service_id = bean.getYOrderServiceId();
                                    // 进入相册
                                    PictureSelector.create(JinXingZhongActivity.this)
...
//                        .setOutputCameraPath()// 自定义相机输出目录，只针对Android Q以下，例如 Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM) +  File.separator + "Camera" + File.separator;
                                            ...
                                            .maxSelectNum(6 - list_img.size())// 最大图片选择数量
...
//                                        .selectionData(mAdapter.getData())// 是否传入已选图片
                ...
                mAdapter_other = new CommonAdapter<OrderDetailModel.OrderInfoBean.OrderGoodsListBean>
                        (JinXingZhongActivity.this, R.layout.item_orderdetail_goods, model.getOrder_info().getOrder_goods_list()) {
                    @Override
...
//                        holder.getView(R.id.delete_goods).setVisibility(View.GONE);
                ...
                mAdapter_jiance = new CommonAdapter<OrderDetailModel.TestingDetailsListBean>(
                        JinXingZhongActivity.this, R.layout.item_orderdetail_jiance, model.getTesting_details_list()) {
                    @Override
...
//                            .apply(RequestOptions.bitmapTransform(new RoundedCorners(10)))
                        ...
                        tv_chageproject.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, AddProjectActivity.class, bundle, false);
                            }
                        });
                        //删除商品
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.ll_feiyong:
                ...
                CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, CouponActivity.class, bundle4, false);
                break;
            case R.id.iv_message:
                ...
                CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, WebContentActivity.class, bundle, false);
                break;
            case R.id.iv_call:
                //拨打电话
...
//                               intent.setAction(Intent.ACTION_DIAL);//只调用拨号界面，不拨出电话
            ...
            case R.id.tv_addservice:
                ...
                CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, AddServiceActivity.class, bundle1, false);
                break;
            ...
            case R.id.tv_addproject:
                ...
                CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, AddProjectActivity.class, bundle3, false);
                break;
...
//                        break;
                ...
                */
                //判断有无项目未完成
                int i = 0;
                for (OrderDetailModel.OrderInfoBean.OrderServiceListBean bean : model.getOrder_info().getOrder_service_list()) {
                    if (bean.getGState() < 2) {
                        i++;
                    }
                }
                if (i > 0) {
                    showToast("您还有" + i + "个项目未完成");
                } else {
                    showToast("确认已完工了吗？", "确认", "取消", new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            dialog.dismiss();
                            Map<String, String> params = new HashMap<>();
                            ...
                            RequestChageType(params, 1, true);
                        }
                    }, new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
            ...
            case R.id.tv_fukuan:
                //返回上一步
                showToast("确认返回上一步吗？", "确认", "取消", new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        dialog.dismiss();
                        Map<String, String> params = new HashMap<>();
                        params.put("u_token", localUserInfo.getToken());
                        params.put("y_techn_sedan_id", y_techn_sedan_id);
                        params.put("g_state", "2");//2为待施工 3为进行中 4待复检  5为待完工 6为已提车  7为付款
                        RequestChageType(params, 2, false);
                    }
                }, new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
            ...
            case R.id.tv_yujitime:
                //预计完成时间
                setDate1("请选择预计完成时间", tv_yujitime, tv_yujitime.getText().toString().trim());
                break;
     ...
     * @param params
     */
    private void RequestChageType(Map<String, String> params, int type, boolean isNext) {
...
//                myToast("修改订单状态成功");
                if (type == 7) {
                    ...
                    if (isNext) {
                        Bundle bundle = new Bundle();
                        bundle.putString("y_techn_sedan_id", y_techn_sedan_id);
                        CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, DaiFuJianActivity.class, bundle, true);
                    } else {
//                        finish();
                        Bundle bundle = new Bundle();
                        bundle.putString("y_techn_sedan_id", y_techn_sedan_id);
                        CommonUtil.gotoActivityWithData(JinXingZhongActivity.this, DaiShiGongActivity.class, bundle, true);
                    }
    ...
    protected void updateView() {
        ...
        titleView.setRightBtn(R.mipmap.ic_scan_black, new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CaptureActivity.launch(JinXingZhongActivity.this, config);
            }
        });
    }
     ...
     * @param params
     */
    private void RequestYuJiTime(Map<String, String> params) {
        OkhttpUtil.okHttpPost(URLs.YuJiTime, params, headerMap, new CallBackUtil() {
            @Override
            public Object onParseResponse(Call call, Response response) {
                return null;
            }
    ...
    private void setDate1(String string, TextView textView, String date) {
        ...
        if (!date.equals("")) {
            ...
            String[] strArr_day = strArr[0].split("-");//拆分日期 得到年月日
            String[] strArr_time = strArr[1].split(":");//拆分日期 得到年月日
            selectedDate.set(Integer.valueOf(strArr_day[0]), Integer.valueOf(strArr_day[1]) - 1, Integer.valueOf(strArr_day[2]), Integer.valueOf(strArr_time[0]), Integer.valueOf(strArr_time[1]));
        }
...
//        calendar.add(Calendar.YEAR, 100);
        endDate.set(year + 10, month, day, hour, minute);
        /*endDate.set(calendar.get(Calendar.YEAR),
                calendar.get(Calendar.MONTH),
                calendar.get(Calendar.DAY_OF_MONTH));*/


        pvTime1 = new TimePickerBuilder(JinXingZhongActivity.this, new OnTimeSelectListener() {
            @Override
            public void onTimeSelect(Date date, View v) {//选中事件回调
                ...
                params.put("u_token", localUserInfo.getToken());
                params.put("y_techn_sedan_id", y_techn_sedan_id);
                params.put("estimate_time", textView.getText().toString().trim());
                RequestYuJiTime(params);
...
//                myToast("上传图片成功");
                ...
                if (list_img_temp != null && list_img_temp.size() > 0) {
                    for (String s : list_img_temp) {
                        imgstr += s + "||";
                    }
                }

                for (String s : response.getList()) {
                    imgstr += URLs.IMGHOST + s + "||";
                }
    ...
    private void RequestUpData(Map<String, String> params) {
        OkhttpUtil.okHttpPost(URLs.ChangeService, params, headerMap, new CallBackUtil() {
            @Override
            public Object onParseResponse(Call call, Response response) {
                return null;
            }
